...
for loc in range(len(collections)):
  data_dict = dict()
  for x in db.collections[loc].find():
    data_dict[x['_id']] = x
  print(f'loc={collections[loc]}, count={len(data_dict)}')
'''

data_dict = dict()
loc = 'Netherlands' #input("Enter the data region: ")

...
dates = []
dates.append(["06-04", "06-05", "06-06", "06-07", "06-08", "06-09", "06-10", "06-11", "06-12", "06-13", "06-14"])
date = dates[int(argv[1])]

# period_choose = argv[1]

streams_end_date = []
streams_end_date.append("06-15")
...
def r_label_2_p2(n_cluster, label):
    file_path = f'./R_capture_probability/n_cluster_{n_cluster}/r_{label}.txt'
...
streams = db.Germany.find({ "start": { "$lt": "2021-"+end_date+"T00:00:00"}, "end": { "$gte": "2021-"+date[0]+"T00:00:00"}})
...
file_cluster = f'./p0_cluster_result/n_cluster_{_num_cluster}_label_{_label}.csv'
...
server_num_each_day = []
for d in date:
    streams = db.Germany.find({ "start": { "$lte": "2021-"+d+"T23:59:59" }, "end": { "$gte": "2021-"+d+"T00:00:00" }})
    cur_ip_list = list()
    for s in streams:
...
for i, d in enumerate(date):
    streams = db.Germany.find({ "start": { "$lte": "2021-"+d+"T"+sample_hour[1]+":00:00" }, "end": { "$gte": "2021-"+d+"T"+sample_hour[0]+":00:00" }})
    cur_ip_list = list()
    for s in streams:
        for (tm, ip) in s["transactionList"].items():
...
output_path = f'./p2_est_result/n_cluster_{_num_cluster}/label_{_label}.txt'
...
plot_path = f'./p2_daily_plot/n_cluster_{_num_cluster}/label_{_label}.png'
